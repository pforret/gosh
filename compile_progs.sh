...
### Created by Peter Forret ( pforret ) on 2022-12-25
...
readonly script_created="2022-12-25"
readonly run_as_root=-1 # run_as_root: 0 = don't check anything / 1 = script MUST run as root / -1 = script MAY NOT run as root
readonly script_description="Compile Go programs in this project"
## some initialisation
action=""
script_prefix=""
script_basename=""
...
choice|1|action|action to perform|compile,action2,check,env,update
...
Script:main() {
  ...
  case $action in
    compile)
      #TIP: use «$script_prefix compile» to ...
      #TIP:> $script_prefix compile
      pushd "$script_install_folder/bin" &> /dev/null || exit
      find "$script_install_folder/src" -type f -name "*.go" \
      | while read -r source_file ; do
          IO:print "Build $(basename "$source_file" .go) ..."
          go build "$source_file"
        done
        popd &> /dev/null || exit
      ;;
